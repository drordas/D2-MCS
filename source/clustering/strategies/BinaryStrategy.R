library("R6")
BinaryStrategy <- R6Class(
  classname = "BinaryStrategy",
  inherit = StrategyGeneric,
  portable = TRUE,
  public = list(
    initialize = function(subset, heuristic, maxClusters = 50) {
      if (!"Subset" %in% class(subset)) {
        stop("[BinaryStrategy][ERROR] subset parameter must be defined as 'Subset' type")
      }
      if ("Heuristic" %in% class(heuristic)) {
        ...
        if (is.list(heuristic) && length(heuristic) >= 1) {
          if (length(heuristic) > 1) {
            warning("[BinaryStrategy][WARNING] SimpleStrategy only use one Heuristic. Assuming the first element on the heuristic list")
          }
          if (!"Heuristic" %in% class(heuristic[[1]])) {
            stop("[BinaryStrategy][ERROR] heuristic parameter must be defined as a 'Heuristic' type")
          } else {
            heuristic <- list(heuristic[[1]])
          }
        } else {
          stop("[BinaryStrategy][ERROR] heuristic parameter must be defined as a 'Heuristic' type or as a list of 'Heuristic' type")
        }  
      }
      if (!is.numeric(maxClusters)) {
        stop("[BinaryStrategy][ERROR] maxClusters parameter must be defined as a 'numeric' type")
      } 
      super$initialize(
        name = "BinaryStrategy",
        subset = subset,
